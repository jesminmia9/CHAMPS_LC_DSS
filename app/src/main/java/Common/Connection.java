...
public class Connection extends SQLiteOpenHelper {
    ...
    //...........................................................................................................
    // Autocomplete texbox event
    //...........................................................................................................
    public void setupAutoComplete(String tableName, AutoCompleteTextView autoCompleteTextView, String columnName){
        autoCompleteTextView.setAdapter(getArrayAdapter("Select distinct "+columnName+" from "+ tableName +" order by "+columnName));
...
//                    Drawable rightDrawavble = autoCompleteTextView.getCompoundDrawables()[DRAWABLE_RIGHT];
    ...
    public static boolean haveNetworkConnection(Context con) {
        ...
        boolean isPoorConnection = false;
        ...
        try {
            ...
            for (NetworkInfo ni : netInfo) {
                if (ni.getTypeName().equalsIgnoreCase("WIFI")) {
                    if (ni.isConnected()) {
                        ...
                        isPoorConnection = isPoorConnection(con, ni.getType());
                    }
                }
            ...
            if (haveConnectedWifi || haveConnectedMobile) {
                isPoorConnection = isPoorConnection(con, ConnectivityManager.TYPE_WIFI) ||
                        isPoorConnection(con, ConnectivityManager.TYPE_MOBILE);
        ...
        return (haveConnectedWifi || haveConnectedMobile) && !isPoorConnection;
    }

    private static boolean isUrlReachable() {
        try {
            URL url = new URL(ProjectSetting.Soap_Address);
            HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
            urlConnection.setConnectTimeout(5000); // Set your desired timeout
    ...
    private static boolean isPoorConnection(Context context, int networkType) {
        try {
            // Check if the required permission is granted before using telephonyManager
            if (ContextCompat.checkSelfPermission(context, Manifest.permission.READ_PHONE_STATE) == PackageManager.PERMISSION_GRANTED) {
                TelephonyManager telephonyManager = (TelephonyManager) context.getSystemService(Context.TELEPHONY_SERVICE);
                if (telephonyManager != null && networkType == ConnectivityManager.TYPE_MOBILE) {
                    int networkClass = getNetworkClass(telephonyManager.getNetworkType());
                    // You can define your own threshold for poor network, for example, NETWORK_CLASS_3G might be considered poor
                    return networkClass == NETWORK_CLASS_2G || networkClass == NETWORK_CLASS_UNKNOWN;
                }
            } else {
    ...
    public static String SelectedSpinnerValue_Index1(String SelectedTest, String SplitValue) {
        String[] D = SelectedTest.split(SplitValue);
        return D[1];
    }
    ...
    public void Sync_Download(String TableName, String Server_TableName, String WhereClause) {
        ...
        request.setTimeStamp(TIMESTAMP==null||TIMESTAMP.length()==0?"":TIMESTAMP);
        ...
        if(response!=null) {
            ...
            String Request_Time_Stamp  = responseData.gettimestamp();
            ...
            try {
                ...
                for (int i = 0; i < totalBatch; i++) {
                    ...
                    if(Res.length()>0) download_success = false;

                    if(i==totalBatch-1 && download_success){
                        SaveData("Update local_index_datasync set timestamp='"+ Request_Time_Stamp +"' where table_name='"+ TableName +"'");
                    }
                }
            } catch (Exception ignored) {
            }
    ...
    private void UploadProcess(String JSON_Data, String TableName, Gson gson){
        ...
        try {
            ...
            if (response != null) {
                ...
                //upload all records as successful upload then update status of upload=2 for unsuccessful
                for (int i = 0; i < responseData.getdata().size(); i++) {
    ...
    public boolean RebuildDatabase(final ProgressDialog progDialog, Handler progHandler, String DeviceID, String UserId) {
        ...
        try {
            ...
            String WorkGroup = ProjectSetting.Get_WorkGroup();

            this.Sync_Download("DeviceList", "DeviceList","DeviceID='"+ DeviceID +"'");
            this.Sync_Download("DatabaseTab", "DatabaseTab","");
            this.Sync_Download("DataCollector", "DataCollector", "UserId='"+ UserId +"' and WorkGroup='"+ WorkGroup +"'");
            this.Sync_Download("DataCollector_AssignArea", "DataCollector_AssignArea", "UserId='"+ UserId +"' and WorkGroup='"+ WorkGroup +"'");
            ...
            this.Sync_Download("GPS_LandmarkType","GPS_LandmarkType", "");
            this.Sync_Download("Religion","Religion", "");
            this.Sync_Download("RTH","RTH", "");
            this.Sync_Download("MS","MS", "");
            this.Sync_Download("Ethnicity","Ethnicity", "");
            progHandler.post(new Runnable() {
                public void run() {
            ...
            this.Sync_Download("OCP","OCP", "");
            this.Sync_Download("EDU","EDU", "");
            this.Sync_Download("MStatus","MStatus", "");
            this.Sync_Download("VaccinationList","VaccinationList", "");
            this.Sync_Download("VaccinationListMaster","VaccinationListMaster", "");
            ...
            this.Sync_Download("Location", "Location", "");
            ...
            progHandler.post(new Runnable() {
                public void run() {
                    ...
                    progDialog.setMessage("Finishing Device Setting ...");
                }
            });
    ...
    public void Sync_DatabaseStructure()
    {
        ...
        if(response!=null) {
            ...
            Sync_DatabaseTab(Final_RequestSQL, TableName, VariableList, UniqueField,Request_Time_Stamp);
        }
    }

    private void Sync_DatabaseTab(String SQL,String TableName,String ColumnList, String UniqueField, String _time_stamp)
    {
        String response = "";
        String TIMESTAMP = _time_stamp; //Global.DateTimeNowYMDHMS();
        try {
            DownloadDataJSON dload = new DownloadDataJSON();
            response = dload.execute(SQL).get();
    ...
    public static void SyncDataService()
    {
        try {
            ...
            C.Sync_Download("Location","Location", "");
            C.Sync_Download("village","village", "");
            C.Sync_Download("GPS_LandmarkType","GPS_LandmarkType", "");
    ...
    @SuppressLint("Range")
    public String Get_VariableList(String TableName) {
        String[] ParaList = null;
        String variable_list = "";
        Cursor cur_H = ReadData("Select t.TableName, t.ColumnList from DatabaseTab t" +
                " where t.tablename='" + TableName + "'");
        cur_H.moveToFirst();

        while (!cur_H.isAfterLast()) {
            variable_list = cur_H.getString(cur_H.getColumnIndex("ColumnList"));
    ...
    public static void LocalizeLanguage(Activity con, int MODULEID, int LANGUAGEID){
        String SQL = "select l.variable_name variablename,v.VariableLabel,l.variable_desc variabledesc,\n" +
                " ifnull(l.ControlType,'') controltype,ifnull(v.Heading,'') heading,l.variable_option variableoption" +
                ...
                if (CONTROL_TYPE.equalsIgnoreCase("label") | CONTROL_TYPE.length() == 0) {
                    ...
                    if (CONTROL_TYPE.equalsIgnoreCase("RadioButton") & VARIABLE_OPTION.length() > 0) {
                        ...
                        if(VARIABLE_OPTION.length()>0) {
                            ...
                            String[] split_spinner_item = null;
                            ...
                            for (int i = 0; i < spinner_item.length; i++) {
                                split_spinner_item = spinner_item[i].toString().split("-");
                                listSpinner.add(spinner_item[i].toString().trim());
                                //listSpinner.add(split_spinner_item[0].trim()+"-"+split_spinner_item[1].trim());
                            }
    ...
    public String NewLandmarkNumber(String EntryUser, String VILLID)
    {
        String LMNO = ReturnSingleValue("Select (Count(*)+1)MaxId from GPS where GPSType='1' and VillID='"+ VILLID +"' and EntryUser='"+ EntryUser +"'");
        return VILLID + EntryUser + Global.Right("000"+LMNO,3);
    }

    public String GetCatchmentArea(String UserId, String WorkGroup)
    {
        String AREA_LIST = "";
        List<String> listItem = new ArrayList<String>();
        listItem = DataListJSON("Select VillID from DataCollector_AssignArea where WorkGroup='"+ WorkGroup +"' and UserId='"+ UserId +"'");
        for(int i=0;i<listItem.size();i++){
            if (i == 0)
                AREA_LIST += "'"+ listItem.get(i).toString() +"'";
            else
                AREA_LIST += ",'" + listItem.get(i).toString() +"'";
